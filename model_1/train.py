...
# 创新点
parser.add_argument('--ent_fusion', default='BertLayer', type=str)
parser.add_argument('--fusion_decoder', default='Transformer', type=str)
...
model = MyGo(num_ent=kg.num_ent, num_rel=kg.num_rel, str_dim=args.str_dim, visual_tokenizer='beit',
             ...
             visual_dropout=args.visual_dropout, textual_dropout=args.textual_dropout, score_function='tucker',
             args=args).cuda()
# 模型加载
# model.load_state_dict(torch.load(f'ckpt/{args.model}/{args.data}/310.ckpt')['state_dict'])

optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
# 优化器加载
# optimizer.load_state_dict(torch.load(f'ckpt/{args.model}/{args.data}/310.ckpt')['optimizer'])

lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=50, T_mult=2)
# 学习率裁剪器加载
# lr_scheduler.load_state_dict(torch.load(f'ckpt/{args.model}/{args.data}/310.ckpt')['scheduler'])
