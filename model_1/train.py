...
from model_1.dataset import VTKG
from model_1.merge_tokens import get_entity_visual_tokens, get_entity_textual_tokens
from model_1.fusion.multi_dimension_fusion import MultiDimensionFusion
from model_1.utils import calculate_rank, metrics
...
parser.add_argument('--model', type=str, default='TwoDimensionFusion')
...
parser.add_argument('--max_vis_token', default=4, type=int)
...
model = MultiDimensionFusion(num_ent=kg.num_ent, num_rel=kg.num_rel, str_dim=args.str_dim, visual_tokenizer='beit',
...
def train_one_epoch(model, optimizer):
    ...
    for batch, label in kg_loader:
        ...
        # ent_emb, rel_embs = model()
        fusion_1d, fusion_2d = model()
        score = model.score(batch.cuda(), fusion_1d, fusion_2d)
        loss = loss_fn(score, label.cuda())
        # if args.mu != 0:
        #     loss += args.mu * model.contrastive_loss_finegrained(ent_emb)
        optimizer.zero_grad()
        loss.backward()
