...
class TransE(nn.Module):
    ...
    def forward(self, ctx_out, rel_out, emb_ent):
        num_ent = len(emb_ent)
        batch_size = len(ctx_out)
        half = batch_size // 2
        rel_out_1, rel_out_2 = rel_out[:half].unsqueeze(1), rel_out[half:].unsqueeze(1)
        tail_out, head_out = ctx_out[:half].unsqueeze(1), ctx_out[half:].unsqueeze(1)  # [batch_size, 1, embedding_dim]
        emb_ent = emb_ent.unsqueeze(0)
        score_1 = emb_ent + rel_out_1 - tail_out
        score_1 = torch.norm(score_1, p=2, dim=-1)
        score_2 = head_out + rel_out_2 - emb_ent
        score_2 = torch.norm(score_2, p=2, dim=-1)
