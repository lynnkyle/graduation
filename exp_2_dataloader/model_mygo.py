...
from model_new import Tucker, ContrastiveLoss, TransE
import numpy as np
...
class MyGo(nn.Module):
    def __init__(self, num_ent, num_rel, str_dim,
        ...
        if self.score_function == 'tucker':
            ...
            self.convtranse_decoder = TransE()
        else:
            pass
        self.init_weights()
    ...
    def score(self, triples, emb_ent, emb_rel):
        ...
        elif self.score_function == 'convtranse':
            score = self.convtranse_decoder(ctx_out, rel_out, emb_ent[:-1])
        else:
